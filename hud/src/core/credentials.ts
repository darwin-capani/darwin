...
export const CRED_HINTS: Readonly<Record<string, string>> = {
  ...
  maps_api_key:
    "Google Maps Platform → Credentials → API key, with Directions, Places, and Distance Matrix enabled. NOT WORKING TODAY — AND IT IS NOT YOUR KEY: VOYAGER calls the LEGACY maps.googleapis.com web services, which read the key ONLY from a ?key= URL parameter, and DARWIN will not put a live credential in a URL (SECURITY.md §8). Google therefore sees no credential at all and answers REQUEST_DENIED, which the status line then blames on your key. Pasting a key here changes nothing until the owner picks a fix — see the KNOWN NON-FUNCTIONAL note at the top of daemon/src/integrations/maps.rs — so do not enable billing on its account expecting routes to start working. VOYAGER reads routes, places, and travel times only — it never books or pays. The key rides the request header, never a logged URL.",
  hibp_api_key:
  ...
  elevenlabs_api_key:
    "elevenlabs.io → Profile → API key. OPTIONAL premium CLOUD voices. THE TIER SHIPS ON AND IS INERT ONLY BECAUSE THIS KEY IS MISSING ([voice].cloud_tier = true): pasting the key is what ACTIVATES it, and from that moment the text DARWIN is about to speak is sent to ElevenLabs to synthesize (it leaves the device). Cloud STT is a SEPARATE switch that ALSO ships on ([voice].cloud_stt = true) and sends your VOICE AUDIO. Turn either off in Settings → System to force the on-device path; Kokoro/whisper stay the private fallback on any error. The key rides the xi-api-key header, never a logged URL. Voice-only — DARWIN keeps its own brain/turn-taking.",
} as const;

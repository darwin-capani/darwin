...
- **Daemon read order at startup:** the `ANTHROPIC_API_KEY` environment variable when set (the existing `state/env.sh` convention keeps working for headless installs and overrides everything), else the Keychain item. Resolution runs **once per process** (`anthropic.rs` caches it in a `OnceLock`), so **restart `darwind` after saving a key from the HUD** — until you do, the daemon keeps the `None` it cached at startup, `cloud_key_present` stays false, the CLOUD KEY light stays amber, and every cloud route silently degrades to the local model. (Same requirement as ARCHITECTURE.md § API key resolution, BRINGUP.md and hud/README.md.)

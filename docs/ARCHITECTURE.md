...
| `[cloud]` | `fast_model = "claude-haiku-4-5"`, `heavy_model = "claude-opus-5"`, `max_tokens = 4096` |
...
Body: `{"model", "max_tokens": 4096, "system", "messages", "tools": [{name, description, input_schema}, ...]}`. Model ids are **exactly** `claude-haiku-4-5` and `claude-opus-5` — no date suffixes; do not append any.
...
3. **Stage + validate EACH candidate independently** — every surviving candidate is copied into its **own** `state/heal/staging-<ts>-c<i>/` (`src/`, `Cargo.toml`, `Cargo.lock` — **never** `target/`), the diff applied with `/usr/bin/patch -p1 --batch` (any failed hunk discards it), then `cargo check` + `cargo clippy --all-targets -- -D warnings` + `cargo test` + a **mutation probe** (the fix is reverse-applied and the patch's own test must then FAIL — a test that passes without its fix is rejected as unproven) under **one budget the whole attempt shares**. A candidate that fails any stage is discarded. **These gates are only ever added to — never weakened.** Every one of them is nonetheless **blind to the diagnosis** — they prove a candidate is *sound*, not that it is an *answer* — so each candidate is additionally scored for **RESPONSIVENESS** (`DIRECT` / `SUBSYSTEM` / `SIGNATURE` / `UNRELATED` / `INDETERMINATE`: does the patch edit the file the burst cited, or the implicated subsystem, or carry the burst's own error text?). That verdict is **advisory and never rejects** — a correct fix often lives one layer up from the line that screamed — but it is written into the validation tail the adversarial reviewer reads, into `report.md`'s header, and onto `heal.proposal` for the HUD's **ADDRESSES** row. **The budget is an attempt-wide ceiling, not a per-candidate allowance.** A candidate may use up to `VALIDATE_TIMEOUT` (900s), but only out of the single attempt budget (`[self_heal].attempt_budget_secs`, default 1800s, clamped to `[750, 14400]`) all three candidates share — so an attempt's worst-case wall clock is the same as the 3 x 600s it replaces, while a slow machine spends it *finishing* one or two real verdicts instead of timing out three times at 87% of the way through. Measured cold on an M1 Pro in a tree staged exactly as the gate stages one: check 92s + clippy 184s + test 214s + mutation re-run 33s = 523s, which left ~77s of headroom under the old 600s — on a machine that, by the definition of self-heal firing, is already misbehaving. Each stage is additionally capped at its share (the floors of the stages behind it are reserved), so a pathologically slow clippy cannot leave `cargo test` three seconds and a deadline. If a budget runs out before a stage returns, the candidate is rejected at stage **`deadline`**, never under the stage it never reached: exhaustion is a capacity failure of the gate, not a verdict on the patch, and the rejection report says WHICH ceiling stopped it (this candidate's budget, one stage's share, or the attempt-wide budget already spent by earlier candidates — a candidate the attempt cannot afford is refused *without being staged at all*).
...
5. **Select — and a CONFIDENCE FLOOR** — among PASSED candidates, the highest review confidence wins; ties break toward the **minimal** patch (smallest add/remove count). **Below the review-confidence floor (`[self_heal].confidence_floor`, default 0.25, clamped to `[0.05, 0.95]`) nothing is proposed at all.** Every staged gate is mechanical and blind to whether the patch is a good *idea*; the adversarial reviewer is the only stage that judges that, and its verdict previously could not stop anything — the best of three patches the reviewer disbelieved was surfaced with an **ACCEPT & APPLY** button beside it, and a failed review call (recorded as 0.0, which is *no review*, not a bad one) was surfaced the same way. A below-floor attempt is its own outcome (`heal.rejected{stage:"confidence"}`), NOT a gate failure, and it still writes every candidate, the review and the diagnosis under `state/heal/rejected/<ts>/` — nothing is lost, there is simply no one-click apply for a patch nobody vouched for. `scripts/apply_heal.sh` enforces the **same** floor on whatever it is asked to install, via `darwind --heal-confidence` (one implementation, two callers — and that CLI loads the same `config/darwin.toml`, so a tuned floor moves both gates together or neither). **Both numbers are CALIBRATABLE, not just settable:** every attempt emits each candidate's review confidence and each cargo stage's real wall time on `heal.proposal` / `heal.rejected` under `calibration`, so after N attempts the right floor and the right budget are read off measurements from *your* machine instead of argued.
6. Outcomes:
   - **8a `mode = "propose"`** (default): write `state/heal/proposals/<ts>/{patch.diff, report.md, diagnosis.json, candidates.md, review.md}` (report = diagnosis, chosen diff, validation-output tail, review verdict + confidence, and the exact apply command), stamp `meta.heal_pending=<ts>`, emit `heal.proposal{ts, files, validated:true, confidence, confidence_floor, responsiveness}` (the floor travels *with* the score so the HUD renders the bar the daemon actually enforces instead of keeping a second copy of the number) (`validated` and `responsiveness` are different claims — the gates are blind to the diagnosis, so an `UNRELATED` patch is `validated:true` too). The proactive first-contact brief reads `meta.heal_pending`, so DARWIN *tells* the user a fix is waiting. A human applies it one of two ways, both **human-gated** and both running the **same re-validation gate**:
     - **Terminal:** `scripts/apply_heal.sh <ts>` — shows the report + diff, asks for confirmation (`read -r`), RE-VALIDATES (fresh staging copy of `daemon/` sources + `/usr/bin/patch -p1 --batch` + `cargo check` + `cargo clippy --all-targets -- -D warnings` + `cargo test` + a **mutation probe** (the fix is reverse-applied and the patch's own test must then FAIL — a test that passes without its fix is rejected as unproven); the split is done by `darwind --split-heal-diff`, the same code the daemon gate uses, so the two gates cannot drift). Between the `cargo test` gate and that mutation probe it **prints the RESPONSIVENESS verdict** re-derived from `darwind --heal-responsiveness` (the daemon's own function again — one implementation, two callers; advisory, it never refuses an apply) and then enforces the **REVIEW-CONFIDENCE FLOOR** (`darwind --heal-confidence` on the proposal's own `report.md` — the daemon's function a third time; this one **does** refuse, because it is the reviewer's explicit verdict on this patch rather than a heuristic about where a fix ought to live, and it stops an older or hand-edited proposal being installed under a weaker bar than the one that would have blocked writing it) — both before the probe, because the probe reverse-applies the patch's fix into the staged crate and leaves it there, and a crate with its fix lifted out often no longer compiles, so no `darwind` could be run from it. It then **only on green** applies to `daemon/`, rebuilds `--release`, clears `meta.heal_pending`, then `launchctl kickstart -k`s the daemon if its service is loaded (else prints "restart darwind manually").
   ...
   - If no candidate passes every gate → `state/heal/rejected/<ts>/` + `heal.rejected{ts, stage}` with `stage ∈ {draft, patch, check, clippy, test, mutation, deadline, apply, build}`. `deadline` is distinct on purpose: it means the validation budget ran out before any gate returned, so *no patch was judged* — the report says so instead of "no candidate passed the gates", which reads as "the model drafted bad patches".

The gates exist because a model-authored patch applied to a privileged daemon is a real risk: nothing is drafted while disabled, nothing touches the live tree in propose mode (the propose path writes artifacts only), `"auto"` is opt-in twice (enabled **and** mode), every patch must survive real `patch` + `cargo check` + `cargo clippy --all-targets -- -D warnings` + `cargo test` + a **mutation probe** (the fix is reverse-applied and the patch's own test must then FAIL — a test that passes without its fix is rejected as unproven) in staging first, nothing below the **review-confidence floor** is proposed or applied at all, and the cadence is capped at one cloud draft per 6h. The HUD's **SELF-REPAIR // PROPOSALS** panel (warn-amber attention state, segmented confidence gauge) is a **human-gated apply surface**, not auto-heal: it fetches and renders the **actual staged diff** for review (`heal_proposal_detail`), and its **ACCEPT & APPLY** button is **two-step** — the first click arms a distinct "CONFIRM — APPLY & REBUILD" state, and only the second click (after a short re-arm window so a double-click cannot skip the confirm) calls `heal_apply`, which runs the gated `scripts/apply_heal.sh <ts> --yes`. The re-validation gate (fresh staging copy + check + clippy + full test + mutation probe + the review-confidence floor) is **mandatory and non-bypassable** — there is no flag that weakens it, and a validation failure refuses to touch `daemon/src` (the UI shows the failure in alert-red; the live code is untouched). The read-only `scripts/apply_heal.sh <ts>` command line is kept on the panel too (the terminal path remains). `self_heal` ships `enabled = true` but **PROPOSE-ONLY** (`mode = "propose"`, inert without a cloud key); this is a human-gated apply of an already-proposed patch, the **only** sanctioned change to the live tree besides the (doubly-opt-in, dangerous) `mode = "auto"`.
...
2. **SBPL profile** — `generate_sbpl` derives a `sandbox-exec` (seatbelt) profile to `state/apps/<name>/<name>.sb`: **`(deny default)`** + the stock `bsd.sb` import, then *only* the manifest's grants — exec of the (canonicalized) interpreter, read of the app dir + runtime install prefix + `fs_read`, write of `fs_write` + the app's own socket, and **no IP network whatsoever** — `(deny network*)` is unconditional, because a direct-egress net scope is not grantable on this OS (SBPL has no host/IP filter) and `net_hosts` is refused at validation; declared egress instead rides the daemon-mediated fetch proxy over an AF_UNIX socket. `sandbox-exec` is the deprecated-but-functional CLI over live kernel seatbelt; the derivation is the durable part. Honest limits (same-UID socket trust, and the unscoped inference socket; the former coarse-host-filtering and DNS side-channel limits are gone with direct egress itself) are catalogued in `docs/SANDBOX.md`.

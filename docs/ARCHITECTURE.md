...
```
 ...
 mic ──▶ audio loop ──▶ VAD (RMS gate: rms_threshold / silence_ms / min_speech_ms)                     │
                ...
                │   self-heal watchdog (gated off by default; propose/auto pipeline when enabled)      │
                └──────────────────────────────────────────────────────────────────────────────────────┘
                                │
                                ▼
...
Pipeline: **audio loop → VAD → instant opener → STT → intent classify → router → (local handler + real actions → streamed `converse` | Anthropic cloud tool loop → per-sentence `speak`) → gapless rodio playback**. The moment the VAD hands over an utterance — before transcription even starts — the daemon opens the reply session, waits the 300 ms opener breath (`[speech].opener_delay_ms`, concurrent with STT), and plays a pre-synthesized acknowledgement WAV from the opener bank (see Voice → Instant acknowledgment), so JARVIS is audibly responding a natural beat after the 350 ms VAD window closes. Local LLM-voiced replies then stream sentence-by-sentence out of one `converse` request into the **same** sink — the first content sentence is audible while the model is still decoding the rest. Every stage emits telemetry events; `pipeline.completed` sums them per utterance (including `first_audio_ms`, the latency the user actually perceives).

## Configuration

Canonical file: `config/jarvis.toml`. Both `jarvisd` and the inference server read it at startup and fall back to hardcoded defaults (identical to the file's values) if it is missing. Sections and keys:
...
| `[speech]` | `engine = "kokoro"` (`"kokoro"` \| `"csm"` \| `"orpheus"` — see TTS engine evaluation), `model = ""` (explicit HF repo; empty = the engine's default), `voice = "bm_george"`, `speed = 1.2` (kokoro only), `sentence_pause_ms = 250` (daemon-inserted silence between consecutive clips), `opener_delay_ms = 300` (the opener breath — see Instant acknowledgment), `openers = ["Right away, sir.", "Of course.", "One moment.", "On it, sir.", "Let me see."]` (instant-acknowledgment bank) |
| `[inference]` | `preload = true` |
| `[self_heal]` | `enabled = false` (master gate; ships OFF), `mode = "propose"` (`"propose"` \| `"auto"` — `"auto"` additionally requires `enabled = true` and is dangerous: it applies the validated patch, rebuilds, and exits the daemon; see Self-heal) |
| `[standing]` | `enabled = false` (master gate; ships OFF — like `[self_heal]`/`[forge]`/`[proactive].speak`; when false the pure scheduler marks NOTHING due so no standing mission ever fires on the live tick — see Standing Missions) |
| `[telemetry]` | `port = 7177` |
...
The HUD's gear → Settings is a **multi-credential registry** with a single source of truth mirrored Rust↔TS (`hud/src-tauri/src/credentials.rs` ↔ `hud/src/core/credentials.ts`). Each entry is `{ id, label, keychain_account, kind }`. v1: **bearer** creds `anthropic` (`anthropic_api_key`), `github` (`github_pat`), `slack` (`slack_bot_token`) — pasted, then **verified live before storing**; **oauth** creds `google_drive` / `google_calendar` — deferred Connect placeholders (not paste-verifiable). Verify-now endpoints: Anthropic `GET /v1/models`, GitHub `GET api.github.com/user` (with `User-Agent`), Slack `POST auth.test`. `verify_and_store(id, secret)` (the Enter action) writes to the Keychain **only when the secret verifies** — an unverified secret is never stored.
...
Intents: `app.launch`, `app.control`, `system.query`, `file.op`, `memory.store`, `memory.recall`, `web.open` (args `{url, browser?}` — the model supplies the canonical domain for well-known sites), `web.search` (args `{query, browser?}`), `conversation`. Both web intents are always `light`. The prompt lives in `inference/prompts/intent_classifier.txt`; its few-shots include the once-failing utterance "Open the official apple website on safari." verbatim.
...
## Self-heal v2 (gated; ships OFF; battle-tested via the heal drill)

Config: `[self_heal] enabled = false` (master gate), `mode = "propose"` (`"propose"` | `"auto"`; unknown values behave as `"propose"`). `"auto"` additionally requires `enabled = true` and is **dangerous by design** — see step 8b. v2 keeps the same two config keys (lockstep `config.rs` ↔ `jarvis.toml`); it adds no new ones.
...
## Optimization-from-usage loop (gated; ships OFF; propose-only; live-capable but OFF by default)

Config: `[optimize] enabled = false` (master gate), `mode = "propose"` (`"propose"` | `"auto"`; `"auto"` is reserved and behaves as `"propose"` — there is **no** auto-apply-to-live path). Same OFF-by-default, propose-only contract as `[self_heal]`/`[forge]`, and the same lockstep `config.rs` ↔ `jarvis.toml` keys.

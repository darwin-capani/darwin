...
3. **Stage + validate EACH candidate independently** — every surviving candidate is copied into its **own** `state/heal/staging-<ts>-c<i>/` (`src/`, `Cargo.toml`, `Cargo.lock` — **never** `target/`), the diff applied with `/usr/bin/patch -p1 --batch` (any failed hunk discards it), then `cargo check` + `cargo clippy --all-targets -- -D warnings` + `cargo test` + a **mutation probe** (the fix is reverse-applied and the patch's own test must then FAIL — a test that passes without its fix is rejected as unproven) under one 10-minute deadline. A candidate that fails any stage is discarded. **These gates are only ever added to — never weakened.** Every one of them is nonetheless **blind to the diagnosis** — they prove a candidate is *sound*, not that it is an *answer* — so each candidate is additionally scored for **RESPONSIVENESS** (`DIRECT` / `SUBSYSTEM` / `SIGNATURE` / `UNRELATED` / `INDETERMINATE`: does the patch edit the file the burst cited, or the implicated subsystem, or carry the burst's own error text?). That verdict is **advisory and never rejects** — a correct fix often lives one layer up from the line that screamed — but it is written into the validation tail the adversarial reviewer reads, into `report.md`'s header, and onto `heal.proposal` for the HUD's **ADDRESSES** row. If the 10-minute budget runs out before a stage returns, the candidate is rejected at stage **`deadline`**, never under the stage it never reached: exhaustion is a capacity failure of the gate, not a verdict on the patch, and the rejection report says which one it was.

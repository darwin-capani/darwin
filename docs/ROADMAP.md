...
- Core ML **embeddings have LANDED and are the DEFAULT** (`[inference].embedder = "coreml-bge-small-en-v1.5"`): BAAI/bge-small-en-v1.5 converted to Core ML (`inference/coreml_embed.py`) serves `op=embed`, ANE-eligible, convert-on-first-use + warmed at preload, with an HONEST fallback to the legacy MLX mean-pool path (`llm-qwen3-4b-meanpool`) that REPORTS it fell back. Measured on the committed eval (`inference/benchmarks/coreml_eval/`): recall@1 0.82 vs 0.25 mean-pool.
- The **learned VAD has LANDED** (`inference/coreml_vad.py` exports the Silero weights at preload; the daemon runs it in-process), replacing the RMS gate.
- **Remaining** for this phase: the **wake-word** model — selection + Core ML export. (ANE probe groundwork landed in Phase 1.5.)
...
**Next — per-agent muscle (each integration needs the user's own credential, added one at a time):** the framework is the skeleton; the muscle is real external reach. Veronica → email/social **send** (Gmail/X/LinkedIn). Steve → **GitHub** (open PRs, read CI). Oracle/Pepper → **Slack** + calendar/**Drive**. Vision/Stark → ads + competitor data sources. Gecko → market data sources (the Algo-Core app that would have fed it is **deleted** — see Phase 4 and `docs/BLOCKED_APPS.md`). Each ships OFF until the user supplies and authorizes the specific credential — no account is assumed, nothing acts on the user's behalf without that explicit grant. Cloud-side allowlist filtering (threading each agent's `tools` into `complete_with_tools`) is a tracked follow-up.
...
| **Gecko** | Markets + capital | Trading/markets. It has **no app feed**: the Algo-Core app it was to tie to is **DELETED and was never built** (see Phase 4 and `docs/BLOCKED_APPS.md`). Engineering only — no profitability claims. |
...
already exists) → Steve/Oracle (ride the tool loop) → Gecko (would have ridden Algo-Core, which
is DELETED — see Phase 4 and `docs/BLOCKED_APPS.md` — so Gecko has no app feed) → Ultron/Vision
...
**⛔ DELETED, NOT "still to build"** — the other two launch apps were spec-only AND **could not be built as specified**, so they are **removed from the tree**. Their manifests declared a `net_hosts` host allow-list, which is **not grantable on this OS at all** (macOS SBPL has no host or IP filter), so both manifests were **REFUSED at validation**: they never loaded, never appeared on the App Deck, and — before the refusal landed — never launched either (their sandbox profile failed to compile, exit 65). Neither could migrate to the supported route (the daemon-mediated fetch proxy, `fetch_hosts`), so each needed a **new mechanism that widens DARWIN's egress posture**. **The owner's decision was `deletion`**, and it is taken: every app under `apps/` now validates, and the app count is a count of apps that can run. The design record is not lost — **`docs/BLOCKED_APPS.md`** carries both specs' purpose, endpoints, blocking mechanism, the mechanism each would need, and the git SHA to recover the full source. Full reasoning: `docs/SANDBOX.md` → *A net scope is not grantable*.

- **Algo-Core** — **DELETED** (`docs/BLOCKED_APPS.md`). Event-driven engine, WASM-sandboxed strategies, walk-forward validation, risk limits + kill-switch, signed SQLite audit log. Engineering spec only — no profitability claims. Blocked because its market data was persistent WebSocket streaming and the one-shot fetch proxy cannot carry a subscription; reviving it needs a daemon-side WebSocket relay. **Heaviest of the two: it places real orders on real venues, so its egress is a financial decision, not just a sandbox one — and the egress grant does not imply the order authority.**
- **Fab-Link** — **DELETED** (`docs/BLOCKED_APPS.md`). Moonraker/OctoPrint websocket telemetry, progress-synced toolpath render, thermal/timelapse panels, Phase-3 ANE vision hook reserved. Blocked because Moonraker is `ws://voron.local:7125/websocket`, which fails the fetch proxy on scheme, port, and the SSRF guard (a `.local` mDNS name is a private LAN address, refused by design); reviving it needs a WebSocket relay or a LAN-scoped SSRF exception.

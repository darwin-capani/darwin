...
| **Gecko** | Markets + capital | Trading/markets; ties to Algo-Core (`apps/algo-core/SPEC.md`) — which is **BLOCKED and unbuilt** (see Phase 4), so this tie is a plan, not a live feed. Engineering only — no profitability claims. |
...
already exists) → Steve/Oracle (ride the tool loop) → Gecko (would ride Algo-Core, which is
BLOCKED — see Phase 4) → Ultron/Vision
...
**⛔ BLOCKED, NOT "still to build"** — the other two launch apps are spec-only AND **cannot be built as specified**. Their manifests declare a `net_hosts` host allow-list, which is **not grantable on this OS at all** (macOS SBPL has no host or IP filter), so both manifests are **REFUSED at validation**: they never load, never appear on the App Deck, and — before the refusal landed — never launched either (their sandbox profile failed to compile, exit 65). Neither can migrate to the supported route (the daemon-mediated fetch proxy, `fetch_hosts`), so each needs a **new mechanism that widens DARWIN's egress posture**. That is an **OWNER DECISION and it has not been taken**; nobody should start implementing against these specs until it is. Full reasoning: `docs/SANDBOX.md` → *A net scope is not grantable*.

- **Algo-Core** (`apps/algo-core/SPEC.md`) — **BLOCKED.** Event-driven engine, WASM-sandboxed strategies, walk-forward validation, risk limits + kill-switch, signed SQLite audit log. Engineering spec only — no profitability claims. Blocked because its market data is persistent WebSocket streaming and the one-shot fetch proxy cannot carry a subscription; unblocking it needs a daemon-side WebSocket relay. **Heaviest of the two: this app places real orders on real venues, so its egress is a financial-risk decision, not just a sandbox one.**
- **Fab-Link** (`apps/fab-link/SPEC.md`) — **BLOCKED.** Moonraker/OctoPrint websocket telemetry, progress-synced toolpath render, thermal/timelapse panels, Phase-3 ANE vision hook reserved. Blocked because Moonraker is `ws://voron.local:7125/websocket`, which fails the fetch proxy on scheme, port, and the SSRF guard (a `.local` mDNS name is a private LAN address, refused by design); unblocking it needs a WebSocket relay or a LAN-scoped SSRF exception.

...
- **IPC.** Newline-delimited JSON (one object per line) over a per-app Unix socket at `state/ipc/apps/<name>.sock`. The daemon creates and owns the socket (bound `0600`, parent dir `0700`); the sandbox profile grants the app access to its own socket path only. The wire protocol (exact, mirrored in `daemon/src/apps.rs`, `apps/global-scan/main.py`, and the HUD reducer):
  - **app → host:** `{"token": <str>, "type": "items"|"status"|"log", "data": <obj>}` — every line carries the capability token.
...
- **`self_heal` ships `enabled = false`.** With the gate off, an error burst only emits `heal.suppressed`; nothing is drafted and nothing touches the live tree.
...
**Ships OFF, with ZERO tools by default.** `[mcp].enabled = false` is the default; with it off, no server connects and no MCP tool exists for any agent. Even with `enabled = true`, the tool surface is empty until you add at least one `[[mcp.servers]]` entry. There is no auto-discovery and no bundled server.
...
**Layer 2 — confirmation gate + OFF-by-default master switch.** A **CONSEQUENTIAL** MCP tool **parks** behind the cross-turn spoken-confirmation gate, identical to the built-in consequential tools, and only acts after the user confirms. Parking is additionally fenced by the master switch `[integrations].allow_consequential`, which ships **OFF**. **Fail-safe classification:** any unknown or mutating MCP tool defaults to CONSEQUENTIAL — a tool is treated as read-only (ungated) **only** when the server config explicitly marks it so.

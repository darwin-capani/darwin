...
Two former launch apps, **Algo-Core** and **Fab-Link**, have been **DELETED**. Their manifests declared `net_hosts`, which is not grantable on this OS at all, so validation refused them: they never loaded, never appeared on the App Deck, and never launched even before the refusal (their profile failed to compile, exit 65). They were not "not built yet" — they could not be built as specified. Carrying them as spec-only directories with ⛔ banners was the cautious call, not the precise one: **every app under `apps/` now validates**, and any count of shipped apps is now a count of apps that can run. What each was for, the exact endpoints each needed, why neither can use the fetch proxy, the mechanism each would need, and the git SHA their full source is recoverable from are in **`docs/BLOCKED_APPS.md`**. See also *A net scope is not grantable* below.
...
**This example is a SHIPPED, RUNNING app.** It used to be `apps/fab-link/manifest.toml` — which was a bad choice on two counts: that app had no implementation, and its manifest was *refused* (see *A net scope is not grantable*), so the schema doc's canonical example was a manifest the daemon rejects and a launch sequence that never happened. (That app is since deleted — `docs/BLOCKED_APPS.md`.) The example below is `apps/global-scan/manifest.toml`, abridged: the first app on the substrate, live today, and the one that demonstrates the *only* supported egress route.
...
#### Owner decision TAKEN — the two apps that could take no route are deleted

`fab-link` and `algo-core` were spec-only (`SPEC.md` + `manifest.toml`, no implementation) and already unlaunchable. Neither could be migrated to the fetch proxy as written:
...
Granting either of them direct egress would require a new mechanism (a daemon-side WebSocket relay, or an explicit LAN-scoped exception to the SSRF guard) — **an owner decision, not a validator change**. `algo-core` additionally **places real orders on real venues** per its spec, which makes its egress a financial-risk decision as much as a sandbox one.

**The decision was `deletion`, and it is taken.** Both directories are removed. Retention-with-banners was the earlier, cautious call; it left the tree carrying two apps that can never run, made every audit re-derive the same conclusion, and made any "shipped apps" count misleading. Git preserves the design record in full, so this is not information loss: **`docs/BLOCKED_APPS.md`** carries what each app was for, the exact endpoints each needed, why each cannot work, the precise mechanism each would need, the financial-risk note for `algo-core`, and the git SHA to recover the full source from.

**The guard did not go with them.** `apps::tests::shipped_manifests_all_validate_and_declared_tools_are_served` used to assert *positively* that each of those two was refused. It now requires **every** manifest under `apps/` to validate, and — because "no shipped app declares a net scope" and "the validator refuses a net scope" are different claims — it additionally injects `net_hosts` into a real shipped manifest that had just validated and requires the refusal. The rule outlives the two examples that happened to trip it, together with `plugin_sdk::tests::a_net_scope_is_refused_as_not_grantable_not_as_over_privileged`, `apps::tests::ceiling_refuses_any_net_hosts_declaration_however_well_formed`, and `apps::tests::a_net_hosts_declaration_is_refused_and_can_no_longer_emit_uncompilable_sbpl` (the one that proves it against the real OS compiler).

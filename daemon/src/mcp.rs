...
//!
//! Connects JARVIS to external MCP tool servers — local subprocesses speaking
...
//! external surface in JARVIS: an MCP server runs code on the user's machine and
...
//!
//!   * PER-AGENT ALLOWLISTED. Each server config lists which JARVIS agents may
...
/// The MCP protocol version JARVIS's client speaks. Sent in `initialize`. MCP
...
/// The outcome of a `tools/call`, mapped into JARVIS-friendly language. Mirrors
...
/// Render a `tools/call` `result` into a JARVIS-friendly outcome. MCP returns
...
pub fn stdio_sandbox_profile(s: &McpServerConfig, project_root: &Path) -> String {
    ...
    p.push_str(&format!(
        ";; Generated by jarvisd for MCP stdio server {:?} — docs/SANDBOX.md.\n",
        s.name
    ));
...
mod tests {
    ...
    async fn disabled_connect_all_is_a_noop() {
        ...
        let mut mgr = McpManager::new(cfg);
        mgr.connect_all(Path::new("/tmp/jarvis-test-root"))
            .await
            .expect("noop");
    ...
    fn allowlist_admits_orchestrator_and_listed_agents_only() {
        ...
        let mgr = McpManager::new(cfg);
        assert!(mgr.agent_may_use("jarvis", "files"), "orchestrator always");
        assert!(mgr.agent_may_use("friday", "files"), "listed agent");
        assert!(!mgr.agent_may_use("veronica", "files"), "unlisted agent denied");
        assert!(!mgr.agent_may_use("jarvis", "nope"), "unknown server denied");
    }
...
\n";
    ...
    async fn http_server_enforces_the_per_agent_allowlist() {
        ...
        assert!(mgr.agent_may_use("jarvis", "ops"), "orchestrator always");
    ...
    fn stdio_sandbox_profile_is_default_deny_and_scopes_to_declarations() {
        ...
        s.net_hosts = vec![];
        let profile = stdio_sandbox_profile(&s, Path::new("/Users/me/jarvis"));
    ...
    async fn disabled_manager_offers_no_mcp_tool_defs() {
        ...
        let mgr = McpManager::new(cfg);
        assert!(mgr.tool_defs_for_agent("jarvis").is_empty(), "disabled -> no defs");
    }
    ...
    fn global_falls_back_to_a_disabled_manager_when_uninstalled() {
        ...
        assert!(g.tool_defs_for_agent("jarvis").is_empty());
    }

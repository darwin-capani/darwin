...
//!      jarvisd, networking tools). A denylisted command is REJECTED PRE-exec and
...
pub fn classify_shell_command(cmd: &str) -> ShellClass {
    ...
    // does not need rm -rf from JARVIS).
    ...
    if (word_present(&norm, "kill") || word_present(&norm, "killall") || word_present(&norm, "pkill"))
        && (norm.contains("jarvis") || norm.contains("-9") || norm.contains("launchd"))
    {
        return ShellClass::Denylisted { reason: "killing the daemon / a process" };
    }
    if word_present(&norm, "launchctl")
        || (word_present(&norm, "kill") && norm.contains("jarvis"))
        || norm.contains("com.jarvis")
    {
...
const PROTECTED_PATHS: &[&str] = &[
    ...
    ".claude", "/state/jarvis", "state/jarvis", "keychain", "secrets",
...
/// denied so a sandboxed command can never read or clobber JARVIS's own state.
pub fn generate_shell_sbpl(scratch_dir: &Path, home: &Path, daemon_state: &Path) -> String {
    let mut s = String::new();

    // --- header ---------------------------------------------------------
    s.push_str("(version 1)\n");
    s.push_str(";; Generated by jarvisd for the sandboxed shell (#43). DENY-DEFAULT:\n");
    ...
    // from the denied state/jarvis db + secrets).
...
mod tests {
    ...
    fn denylist_rejects_destructive_commands() {
        let dangerous = [
            ...
            "kill -9 jarvisd",
            "killall jarvisd",
            "launchctl unload com.jarvis.daemon",
            "shutdown -h now",
            "reboot",
        ];
    ...
    fn sbpl_profile_is_deny_default_no_net_and_confined() {
        let scratch = PathBuf::from("/Users/me/jarvis/state/shell/scratch/1700");
        let home = PathBuf::from("/Users/me");
        let daemon_state = PathBuf::from("/Users/me/jarvis/state");
        ...
        assert!(
            profile.contains("(allow file-write* (subpath \"/Users/me/jarvis/state/shell/scratch/1700\"))"),
        ...
        assert!(
            profile.contains("(deny file-read* file-write* (subpath \"/Users/me/jarvis/state\"))"),

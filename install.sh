...
# --- prune what a PREVIOUS install shipped and this one no longer does --------
#
# rsync runs WITHOUT --delete, so until now a file deleted from the repo lived in
# the install home forever. MEASURED on the owner's machine: apps/fab-link and
# apps/algo-core were removed from the source in #237 and were still deployed
# three weeks later, making the loader log "skipping invalid micro-app manifest"
# on every boot.
#
# A blanket --delete is NOT the fix and must not be added: the install home also
# receives files DARWIN itself creates that are absent from source — most
# importantly apps/ generated by forge, which a delete would destroy. The
# distinction is not "in source vs not"; it is "SHIPPED BY A PREVIOUS INSTALL vs
# created at runtime", and only a record written at install time can tell them
# apart.
#
# So: each install writes the manifest of what it placed, and the next install
# removes only paths the LAST manifest claims and the current source no longer
# has. A file that never appeared in a manifest is never touched, so
# forge-generated apps and every runtime artifact are safe by construction. A
# missing/corrupt manifest prunes NOTHING (first install, or an install predating
# this) — the failure direction is "leave it", never "delete it".
SHIPPED_MANIFEST="$DARWIN_HOME/state/.shipped-manifest"

prune_unshipped() {
    [ -r "$SHIPPED_MANIFEST" ] || { ui_note "no previous shipped-manifest — pruning nothing"; return 0; }
    local pruned=0 rel old
    while IFS= read -r rel; do
        # Defensive: never act on an empty line, an absolute path, or one that
        # escapes the install home. The manifest is ours, but a tree that
        # `rm -rf`s from a file deserves the same scrutiny as one that does not.
        case "$rel" in ""|/*|*..*) continue ;; esac
        [ -e "$SRC_ROOT/$rel" ] && continue          # still shipped
        old="$DARWIN_HOME/$rel"
        [ -e "$old" ] || continue                    # already gone
        rm -rf "$old" && pruned=$((pruned + 1))
    done < "$SHIPPED_MANIFEST"
    [ "$pruned" -gt 0 ] && ui_ok "pruned $pruned path(s) this build no longer ships" || true
    return 0
}

write_shipped_manifest() {
    mkdir -p "$(dirname "$SHIPPED_MANIFEST")"
    # TOP-LEVEL ENTRIES ONLY under the directories a deletion realistically
    # happens in. Recording every file would make the manifest enormous and would
    # prune individual files out from under a directory the daemon writes into;
    # an app or a script is removed as a unit.
    : > "$SHIPPED_MANIFEST.tmp"
    for d in apps scripts boot docs; do
        [ -d "$SRC_ROOT/$d" ] || continue
        (cd "$SRC_ROOT/$d" && ls -1) 2>/dev/null | while IFS= read -r e; do
            [ -n "$e" ] && printf '%s/%s\n' "$d" "$e" >> "$SHIPPED_MANIFEST.tmp"
        done
    done
    mv -f "$SHIPPED_MANIFEST.tmp" "$SHIPPED_MANIFEST"
}
...
if [ "$MODE" = "check" ]; then
    ...
    if [ "$SRC_ROOT" = "$DARWIN_HOME" ]; then
        ...
        # Prune BEFORE the copy: the manifest describes the PREVIOUS install, so
        # it must be consulted while that install is still on disk.
        prune_unshipped
